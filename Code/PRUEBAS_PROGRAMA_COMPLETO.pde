...
char name[] = "COM3";  //<-----------------INTRODUCIR NOMBRE
char clientName[6];
char myIP[15];
char myServerIP[15];
char myAlarm[4];
...
void client() {
  ...
  if (WIFI.join(ESSID)) {
    ...
    if (WIFI.setTCPclient(myServerIP, SERVER_PORT, CLIENT_PORT)) {
      ...
      // Envía de datos al servidor
      USB.print(F("Sending data: "));
      snprintf(body, sizeof(body), "%s&1->Datos recopilados a las %s/00", name, RTC.getTime());
      USB.println(body);
      WIFI.send(body);
...
void sendMessageToServer() {
  ...
  if (WIFI.join(ESSID)) {
    ...
    // Configuración del envío de datos al host
    for (int k = 0; k<507; k++) {
      if (message[k]==' ') {
        message[k] = '_';
      } else if (message[k]=='/' && message[k+1]=='0' && message[k+2]=='0') {
        memset(message, '\0', k);
      }
    }
